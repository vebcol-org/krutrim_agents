...
| [`openapi.json`](openapi.json) | Static snapshot of the backend's OpenAPI spec — every route's request/response schema. The live version is always served at `GET /openapi.json` (Swagger UI at `/docs`); re-run `uv run python scripts/export_openapi.py` from `backend/` to refresh this file after changing a route's request/response shape. |
