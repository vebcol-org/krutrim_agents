...
**`GET /agents/{agent_id}/health`** → `{"status": "ok", "agent": {"id": agent_id, "agent_key": ...}}`; 404 if unknown or not visible (same `_check_agent_key_visible` gate).
...
`POST /api/chat`, body `ChatMessageRequest {message, chat_id?, session_id?, project_id?, chat_title?, provider?, model?}` → `{chat_id, session_id, message: {role: "assistant", content}}`. Single request/response JSON — **no streaming, no tools, no subagents, no sandbox**.
